...
test_that("when old file and new file are equal return the same hash", {
    # create a temporary file
...
test_that("when old file and new file are not equal return different hashes", {
    # create a temporary file
    old_file <- tempfile(fileext = ".csv")
    ...
    # the warning message is generate by the compare_file_hashes function
    # when the hashes do not match, so we suppress the warning
    # to avoid a false positive in our tests
    # we already explicitly check for the warning message above
